...
```python
# 升级Paddle2.0版本（根据硬件环境选择Paddle版本）

# # 安装gpu环境版本（首次执行后注释）
# !pip uninstall -y paddlepaddle-gpu
# !pip install paddlepaddle_gpu==2.0.0b0.post97 -f https://paddlepaddle.org.cn/whl/stable.html
...
loader_a = DataLoader(h2zdateset_a, places=PLACE, shuffle=True, batch_size=BATCH_SIZE, drop_last=False)

# 定义B组图片loader
h2zdateset_b = H2ZDateset(DATA_DIR, ab_set='b')
loader_b = DataLoader(h2zdateset_b, places=PLACE, shuffle=True, batch_size=BATCH_SIZE, drop_last=False) 
...
class ConvTransIN(nn.Layer):
    def __init__(self, num_channels, num_filters, filter_size, stride=1, padding='same', padding_mode='constant', 
        ...
        model = [
            nn.ConvTranspose2d(num_channels, num_filters, filter_size, stride=stride, padding=padding, 
                bias_attr=bias_attr, weight_attr=weight_attr),
            nn.InstanceNorm2d(num_filters),
            nn.LeakyReLU(negative_slope=0.2)
        ]
...
def train(place, epoch_num=99999, adv_weight=1, cycle_weight=10, identity_weight=10, \
          ...
          print_interval=100, max_step=500, model_bkp_interval=2000):
...
# 继续训练
train(PLACE, print_interval=10, max_step=50, load_model=True)

```

    Start time : 2020-10-23 23:43:47 start step: 86101
    [86110] DA: [0.09782735] DB: [0.05217161] GA: [3.4506369] GB: [2.0257983] 2020-10-23 23:43:50
    [86120] DA: [0.05246627] DB: [0.01457392] GA: [2.7825599] GB: [2.7054362] 2020-10-23 23:43:52
    [86130] DA: [0.16151135] DB: [0.0430919] GA: [2.4978347] GB: [2.3715374] 2020-10-23 23:43:54
    [86140] DA: [0.10874038] DB: [0.0745535] GA: [2.7180097] GB: [2.1122155] 2020-10-23 23:43:56
    [86150] DA: [0.17712027] DB: [0.04740692] GA: [2.4283862] GB: [2.2524238] 2020-10-23 23:43:58
    End time : 2020-10-23 23:44:02 End Step: 86150
